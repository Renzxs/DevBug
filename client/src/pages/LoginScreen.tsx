...
function LoginScreen(){
    ...
    const { username, email, password, setUsername, setEmail, setPassword } = useUserStore();
    ...
    return (
        <Box sx={{width: '100%', height: '100vh', bgcolor: '#F8F8F8', display: 'flex', justifyContent:'center', alignItems: 'center'}}>
            <Box sx={{display: 'flex', justifyContent:'center', alignItems: 'center', flexDirection: 'column', gap: 2, bgcolor: 'white', px: 5, py: 4, borderRadius: '10px'}}>
                ...
                <Typography variant="caption">Didn't have an account yet? <Link sx={{textDecoration: 'none', color: 'black', fontWeight: 'bold', cursor:'pointer'}}>Create an account</Link></Typography>
            </Box>
        </Box>
    )
}
